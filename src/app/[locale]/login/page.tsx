      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 pt-10 pb-20">
        ...
        <div className="w-full max-w-[420px] bg-[#0e0e11] border border-white/5 rounded-3xl p-8 sm:p-10 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <div className="relative flex items-center py-4 mb-4">
            ...
            <span className="flex-shrink-0 mx-4 text-zinc-400 text-xs font-medium">{t('or_email')}</span>
            <div className="flex-grow border-t border-white/5"></div>
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
            <div className="space-y-1.5">
              <label htmlFor="login-email" className="text-xs font-bold text-zinc-400 block text-start">{t('email_label')}</label>
            ...
            <div className="space-y-1.5">
              <label htmlFor="login-password" className="text-xs font-bold text-zinc-400 block text-start">{t('password_label')}</label>
              <div className="relative">
                <div className="absolute inset-y-0 start-0 flex items-center ps-4 pointer-events-none text-zinc-500">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            >
              <span>{t('submit')}</span>
              {/* the arrow points "forward", which is left in RTL and right in LTR */}
              <svg className="w-4 h-4 transform ltr:rotate-180 group-hover:-translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="mt-8 text-center text-sm font-medium">
            <span className="text-zinc-400">{t('start_now')}</span>
            <Link href="/quiz" className="text-rose-500 hover:text-rose-400 transition-colors">{t('start_quiz')}</Link>
          </div>
        ...
        <div className="mt-8 text-xs text-zinc-400 font-medium flex flex-col gap-1 items-center">
          <span>
            {t.rich('agree', {
              terms: (chunks) => <Link href="/terms" className="text-indigo-400 hover:underline">{chunks}</Link>,
              privacy: (chunks) => <Link href="/privacy" className="text-indigo-400 hover:underline">{chunks}</Link>,
            })}
          </span>
        </div>
      </div>
